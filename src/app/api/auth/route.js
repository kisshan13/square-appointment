...
export async function POST(request) {
    try {
        ...
        const customer = await square.customersApi.searchCustomers({ query: { filter: { emailAddress: { exact: payload.email } } } });

        if (!customer.result.customers) {
            return Response.json({ error: "Invalid credentials" }, { status: 400 });
        }

        const isPasswordCorrect = bcrypt.compareSync(payload.password, customer.result.customers.at(0)?.note);
